...
- **v2.9** - 고급 대시보드 & 시각화
- **v2.10** - 보안/유지보수 리팩토링 & 멀티 계정 감시
- **v2.10.1** - Mantis 방법론 기반 보안 감사: 크로스 계정 ExternalId 일관화·크레덴셜 캐시 무효화·예약 IP 탐지·Discord 재생공격 방어 ✅ **CURRENT**
...
- ✅ **보안 강화 (2026-07-16, Mantis 방법론 기반 감사):** 모든 크로스 계정 `sts:AssumeRole` 경로에 `ExternalId`(혼동된 대리인 방어) 일관 적용, 제거된 계정의 임시 크레덴셜 캐시 정확 무효화, `ipaddress` 기반 예약/링크로컬(169.254.0.0/16 메타데이터) IP 탐지, Discord 웹훅 서명 timestamp 재생공격 방어
...
**최종 갱신:** 2026-07-16  
**상태:** 🛠️ 개발 진행 중 · 2,372 테스트 통과 · Mantis 방법론 기반 보안 감사 강화 (크로스 계정/크레덴셜/IP/재생공격)  
