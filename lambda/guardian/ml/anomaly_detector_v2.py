...
def detect_anomaly_sync(metrics: Dict[str, float]) -> Dict[str, Any]:
    """이상 탐지 (sync) - runs the async detector synchronously."""
    import asyncio

    try:
        asyncio.get_running_loop()
        # Already in an async context — offload to thread
        import concurrent.futures

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(asyncio.run, _detector.detect_anomaly(metrics)).result()
    except RuntimeError:
        return asyncio.run(_detector.detect_anomaly(metrics))
