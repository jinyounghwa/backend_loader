...
class OptimizationSuggester:
    ...
    async def suggest_optimizations(self, findings: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        비용 절감 최적화 제안 생성
    ...
    async def _find_unused_resources(self, findings: Dict[str, Any]) -> List[Dict[str, Any]]:
        """미사용 리소스 식별"""
        suggestions = []
        ...
        return [
            s for s in suggestions if s["potential_savings"] >= self.min_monthly_savings_threshold
        ]
    ...
    async def _suggest_reserved_instances(self, findings: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def _suggest_scaling_optimizations(
        ...
        return [
            ...
            if s.get("potential_savings", 0) >= self.min_monthly_savings_threshold
        ]
...
def suggest_optimizations_sync(findings: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            return pool.submit(asyncio.run, _suggester.suggest_optimizations(findings)).result()
    except RuntimeError:
        return asyncio.run(_suggester.suggest_optimizations(findings))
