    ...
    def get_rule_effectiveness(self, rule_id: str, days: int = 30) -> Optional[Dict]:
        try:
            ...
            return {
                ...
                "effectiveness_score": (
                    ((succeeded / total) * 0.6 + (resolved / total) * 0.4) if total > 0 else 0
                ),
            }
        except Exception as e:
                ...
                metrics.append(
                    {
                        "rule_id": rule_id,
                        "action_type": (
                            actions[0].get("action_type", "unknown") if actions else "unknown"
                        ),
                        "total_actions": total,
                        "successful_actions": succeeded,
                        "success_rate": (succeeded / total) if total > 0 else 0,
                        ...
                        "resolution_rate": (resolved / total) if total > 0 else 0,
                        "effectiveness_score": (
                            ((succeeded / total) * 0.6 + (resolved / total) * 0.4)
                            if total > 0
                            else 0
                        ),
                    }
