...
def get_status() -> dict:
    try:
        ...
        running_instances = sum(
            len(r.get("Instances", [])) for r in ec2_response.get("Reservations", [])
        )
...
def stop_instance(instance_id: str) -> dict:
    if not INSTANCE_ID_RE.match(instance_id):
        return {
            "error": f"Invalid instance ID format: {instance_id}",
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
    try:
        ec2 = AWSClientProvider.get_client("ec2")
...
def set_threshold(amount: str) -> dict:
    try:
        ...
        ssm.put_parameter(
            Name="/guardian/cost-threshold", Value=str(threshold), Type="String", Overwrite=True
        )
...
def export_events(format_str: str, days: int = 7, severity: Optional[str] = None) -> dict:
    ...
    try:
        ...
        events = query_events_with_pagination(
            storage.table, severity_filter=severity, days=days, limit=500
        )

        if not events:
            return {"error": f"No events found for past {days} days"}

        summary = {"total_events": len(events), "by_severity": {}, "by_type": {}}
        for event in events:
