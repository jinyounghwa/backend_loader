...
class AutoRemediationResponder:
    ...
    def __init__(
        self, logger: Logger, storage: DynamoDBStorage, telegram: Optional[TelegramResponder] = None
    ):
        self.logger = logger
    ...
    def _execute_action(
        self, action_type: str, resource_id: str, region: str, rule_id: str, dry_run: bool = False
    ) -> bool:
        """Execute an action and log the result."""
        if dry_run:
            self.logger.info(
                "[DRY-RUN] Would execute %s on %s in %s (rule: %s)",
                action_type,
                resource_id,
                region,
                rule_id,
        ...
        try:
            ...
            log_remediation(
                self.logger, action_type, resource_id, "success" if success else "failed"
            )
            return success
    ...
    def _match_event_to_rule(
        self, event_type: str, region: str, resource_region: str
