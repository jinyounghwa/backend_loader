...
class AlertMessage:
    ...
    def from_ec2_data(cls, data: Dict[str, Any], account_info: str = '') -> 'AlertMessage':
        ...
        for exp in data.get('exposed_instances', []):
            items.append({
                ...
                'details': [f"Port {r.get('from_port', '?')}/{r.get('protocol', '?')}" for r in exp.get('exposed_rules', [])[:2]],
            })
    ...
    @classmethod
    def from_generic(cls, check_name: str, data: Dict[str, Any], account_info: str = '') -> 'AlertMessage':
        return cls(
    ...
    @classmethod
    def from_check_data(cls, check_name: str, data: Dict[str, Any], account_info: str = '') -> 'AlertMessage':
        builders = {
